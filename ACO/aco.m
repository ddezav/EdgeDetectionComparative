img =imread('C:\Users\ddeza\Downloads\Edge-Detection-Ant-Colony-Optimization--master\prueba.png');
K = rgb2gray(img);
img1 = imresize(K,[200 200]);
...
X=size(img);%finds the size of the image
row=X(1,1);%total rows
col=X(1,2);%total columns

 C=0.1;
 T=0;

 img2=zeros(row,col);%for the result image edge detected
 a=zeros(3,8);
for i=1:row
    for j=1:col
